<?php
	...
	if(count($_REQUEST)===0) {
		echo "<!DOCTYPE html><html><head><title>Error!</title></head>",
			...
			"<a href='lab7_1_1_A.php'>here</a> and enter your data!</p></body></html>";
		die();
	}
	...
	if(($name==="")||($age==="")||($phone==="")) 
	{
		echo "<!DOCTYPE html><html><head><title>Error!</title></head>",
			...
			"<a href='lab7_1_1_A.php'>here</a> and enter your data again, please!</p></body></html>";
		die();
	}
?>
	...
	<body>
		...
		<?php
			...
			if((!ctype_upper($name[0])) || (!ctype_lower( mb_substr($name, 1) )) ) {
				echo "<p>A name must starts with an uppercase letter and after that it must has only lowercase letters!</p>"; 
				$printData = false;
			}
			/* ----- Age's checking ----- */
			if (!ctype_digit($age)) {
...
#			else echo "<p>DEBUG - The number is right!</p>";
		...
		?>
		<p>	<a href="lab7_1_1_A.php">Go back</a> </p>
