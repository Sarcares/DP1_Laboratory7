<?php
	...
	if(count($_GET)===0) {
		echo "<!DOCTYPE html><html><head><title>Error!</title></head>",
			...
			"<a href='lab7_2_3_A.php'>here</a> and enter your data!</p></body></html>";
		die();
	}
	...
	if( ($id==="")||($quantity==="") ) 
	{
		echo "<!DOCTYPE html><html><head><title>Error!</title></head>",
			...
			"<a href='lab7_2_3_A.php'>here</a> and enter your data again, please!</p></body></html>";
		die();
	}
	...
	<body>
		<?php
			...
			if(($id < 1) || ($id > $totalRows)) {
				...
				if(($quantity<1) || ($quantity>$availableQnt)) {
					echo "<p>Do you have inserted an invalid Quantity! It must be between 1 and $availableQnt</p>";
					$printData = false;
				}
				$price = $result['Prezzo'];
...
#			echo "DEBUG - total rows: $totalRows \t available quantity: $availableQnt";
		...
		?>
		<p>	<a href="lab7_2_3_A.php">Go back</a> </p>
